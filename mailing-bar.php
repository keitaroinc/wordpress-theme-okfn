  ...
  <div class="tabbable mailing-lists">
    <div class="container">
      <ul class="nav nav-tabs">
        <?php if ($okfn_mailchimp_bar == "true") : ?><li class="mailtab<?php echo ++$mailtab_id ?>"><a href="#tab<?php echo $mailtab_id ?>" data-toggle="tab"><?php
          if (!empty($okfn_mailchimp_heading)) {
            echo $okfn_mailchimp_heading;
          } else {
            echo 'Mailing List ' . $mailtab_id;
          }
          ?></a></li><?php endif; ?>
        <?php if ($okfn_mailinglist_bar == "true") : ?><li class="mailtab<?php echo ++$mailtab_id ?>"><a href="#tab<?php echo $mailtab_id ?>" data-toggle="tab"><?php
          if (!empty($okfn_mailinglist_heading)) {
            echo $okfn_mailinglist_heading;
          } else {
            echo 'Mailing List ' . $mailtab_id;
          }
          ?></a></li><?php endif; ?>
        <?php if ($okfn_mailinglist_bar2 == "true") : ?><li class="mailtab<?php echo ++$mailtab_id ?>"><a href="#tab<?php echo $mailtab_id ?>" data-toggle="tab"><?php
          if (!empty($okfn_mailinglist_heading2)) {
            echo $okfn_mailinglist_heading2;
          } else {
            echo 'Mailing List ' . $mailtab_id;
          }
          ?></a></li><?php endif; ?>
        ...
        if ($okfn_mailchimp_bar == "true") {
          ...
          <div class="tab-pane mailchimp-plugin" id="tab<?php echo ++$mail_id ?>">
            <div>
              <?php
              if (!empty($okfn_mailchimp_heading)) {
                echo '<span class="heading">' . $okfn_mailchimp_heading . ' </span>';
              } if (!empty($okfn_mailchimp_description)) {
                echo '<span class="description">' . $okfn_mailchimp_description . '</span>';
              }
                ...
                <form action="<?php echo $okfn_mailinglist_action ?>" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>
                  <table width="100%">
                    <tr>
                      <th scope="row"><?php
                        if (!empty($okfn_mailinglist_heading)) {
                          echo '<span class="heading">' . $okfn_mailinglist_heading . ' </span>';
                        } if (!empty($okfn_mailinglist_description)) {
                          echo '<span class="description">' . $okfn_mailinglist_description . '</span>';
                        }
                        ?></th>
                ...
                <form action="<?php echo $okfn_mailinglist_action2 ?>" method="post" id="mc-embedded-subscribe-form2" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>
                  <table width="100%">
                    <tr>
                      <th scope="row"><?php
                        if (!empty($okfn_mailinglist_heading2)) {
                          echo '<span class="heading">' . $okfn_mailinglist_heading2 . ' </span>';
                        } if (!empty($okfn_mailinglist_description2)) {
                          echo '<span class="description">' . $okfn_mailinglist_description2 . '</span>';
                        }
                        ?></th>
                      <td><label for="mce-EMAIL">Email Address </label>
                        <input type="email" value="" name="EMAIL" class="required email" id="mce-EMAIL" placeholder="Email Address"></td>
